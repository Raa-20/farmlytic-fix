...
export default function Sidebar({ setMenu }) {
  const handleLogout = async () => {
    try {
      ...
      await axios.post("http://10.75.133.16:5000/logout", {
        username: storedUser,
        role: storedRole
      });
    } catch (error) {
  ...
  return (
    <div className="w-64 bg-green-700 text-white h-screen flex flex-col">
      <div className="h-16 shrink-0 flex items-center px-6 border-b border-white gap-3">
        <img src={myLogo} alt="Logo Ladentra" className="w-10 h-10 object-contain bg-white rounded-full p-1" />
        <h1 className="text-xl font-bold tracking-wide">Ladentra</h1>
      </div>

      <div className="p-4 flex-1">
        <button onClick={() => setMenu("dashboard")} className="block mb-4 text-left w-full hover:text-green-200">
          Dashboard
        </button>
        <button onClick={() => setMenu("users")} className="block mb-4 text-left w-full hover:text-green-200">
          User
        </button>
        <button onClick={() => setMenu("master")} className="block mb-4 text-left w-full hover:text-green-200">
          Master Data
        </button>
        <button onClick={() => setMenu("audit")} className="block text-left w-full hover:text-green-200">
          Audit Log
        </button>
      </div>
